...
- list[0]为插件名，插件必须在`pkg/plugin/Plugin.py`或`custom/plugin/Plugin.py`中存在，不存在将会引发异常
- list[1]为插件所需参数，类型为map/dict，键为参数名，值为参数值，**以参数名命名的类**必须存在于相应的插件类构造函数中，否则会引发异常
